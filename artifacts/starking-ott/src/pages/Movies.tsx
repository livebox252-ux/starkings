...
export default function Movies() {
  useSeo({
    title: "Browse Movies",
    description: "Discover the latest and greatest movies on Star King OTT. Filter by your favorite genres.",
  });
  ...
  return (
    <AppLayout>
      <div className="pt-24 pb-12 px-4 md:px-6 container mx-auto">
        <h1 className="text-4xl md:text-5xl font-serif font-bold text-white mb-8">Movies</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
          {isLoading ? (
            ...
            data?.results?.map((item) => (
              <MediaCard key={item.id} {...item as any} media_type="movie" />
            ))
          )}
        </div>
